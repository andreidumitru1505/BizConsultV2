...
const CompanyDashboard = () => {
    ...
    useEffect(() => {
        console.log(state);
        fetch('http://localhost:8080/getCompanyDashboardInfo',{
            method: 'POST',
            headers:{
            ...
            .then(data => {setCompanyInfo(data);setIsLoading(0)})
                    ...
                    <aside class="flex px-16 space-y-16 overflow-hidden m-3 pb-4 flex-col items-center justify-center rounded-tl-xl rounded-bl-xl bg-blue-800 shadow-lg">
                        ...
                        <li class="flex space-x-2 mt-4 px-6 py-4 text-white hover:bg-white hover:text-blue-800 font-bold hover:rounded-br-3xl transition duration-100 cursor-pointer">
                            ...
                            <button  onClick={() => navigate("/partnerSearch",
                                            {state:{firstName:state.firstName, lastName:state.lastName, emailAddress:state.emailAddress, role:state.role, companyId: companyId, companyName: companyInfo.name}})}>
                                            Add Partner
                            </button>
                        </li>
                        <li class="flex space-x-2 mt-4 px-6 py-4 text-white hover:bg-white hover:text-blue-800 font-bold hover:rounded-br-3xl transition duration-100 cursor-pointer">
                            ...
                            <button  onClick={() => navigate("/collaborations",
                                            {state:{firstName:state.firstName, lastName:state.lastName, emailAddress:state.emailAddress, role:state.role, companyId: state.companyId, companyName: companyInfo.name}})}>
                                            Collaborations
                            </button>
                        </li>
                        <li class="flex space-x-2 mt-4 px-6 py-4 text-white hover:bg-white hover:text-blue-800 font-bold hover:rounded-br-3xl transition duration-100 cursor-pointer">
                    ...
                    <div class="flex p-4 bg-white items-center mt-3 rounded-xl shadow-lg">
                        ...
                        <div class="flex justify-between mt-4 space-x-4 s">
                            ...
                            <div class="bg-white w-1/3 rounded-xl shadow-lg flex items-center justify-around">
                                ...
                                <div class="text-center">
                                    <h1 class="text-4xl font-bold text-gray-800">{companyInfo.rating}</h1>
                            ...
                            <div class="bg-white w-1/3 rounded-xl shadow-lg flex items-center justify-around">
                                ...
                                <div class="text-center">
                                    <h1 class="text-4xl font-bold text-gray-800">{companyInfo.earnings}</h1>
                                    <span class="text-gray-500">Collaborations Earnings</span>
                                </div>
                            </div>
                        </div>
                    </main>
